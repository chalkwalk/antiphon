...
template <class T> static int WDL_assocarray_cmp(const T *a, const T *b) {
  return *a > *b ? 1 : *a < *b ? -1 : 0;
}
template <class T> static int WDL_assocarray_cmpmem(const T *a, const T *b) {
  return memcmp(a, b, sizeof(*a));
}
template <class T> static int WDL_assocarray_cmpstr(T *const *a, T *const *b) {
  return strcmp(*a, *b);
}
template <class T> static int WDL_assocarray_cmpistr(T *const *a, T *const *b) {
  return stricmp(*a, *b);
}

// on all of these, if valdispose is set, the array will dispose of values as
// needed. if keydup/keydispose are set, copies of (any) key data will be
// made/destroyed as necessary

// WDL_AssocArrayImpl can be used on its own, and can contain structs for keys
// or values
...
public:
  ...
  int Insert(KEY key, VAL val) {
    ...
    if (ismatch) {
      ...
      if (WDL_NORMALLY(kv != NULL)) {
        memmove(kv + i + 1, kv + i,
                (m_data.GetSize() - i - 1) * sizeof(KeyVal));
        if (m_keydup)
          key = m_keydup(key);
        kv[i].key = key;
        kv[i].val = val;
  ...
  void ChangeKey(KEY oldkey, KEY newkey) {
    ...
    if (ismatch)
      ChangeKeyByIndex(i, newkey, true);
  }
  ...
  void SetGranul(int gran) { m_data.SetGranul(gran); }
  ...
  void CopyContents(const WDL_AssocArrayImpl &cp) {
    ...
    m_valdispose = NULL; // avoid disposing of values twice, since we don't have
                         // a valdup, we can't have a fully valid copy
    if (m_keydup) {
  ...
  // private data, but exposed in case the caller wants to manipulate at its own
  // risk
  struct KeyVal {
    KEY key;
    VAL val;
...
private:
  void RemoveDuplicateKeys() // after resorting
  {
    ...
    if (cnt < sz)
      m_data.Resize(cnt, false);
  }
};

// WDL_AssocArray adds useful functions but requires assignment operator for
// keys and values
template <class KEY, class VAL>
class WDL_AssocArray : public WDL_AssocArrayImpl<KEY, VAL> {
public:
...
template <class KEY, class VAL>
class WDL_KeyedArray : public WDL_AssocArray<KEY, VAL> {
public:
  explicit WDL_KeyedArray(void (*valdispose)(VAL) = NULL)
      : WDL_AssocArray<KEY, VAL>(WDL_assocarray_cmp<KEY>, NULL, NULL,
                                 valdispose) {}
};

template <class KEY, class VAL>
class WDL_KeyedArrayImpl : public WDL_AssocArrayImpl<KEY, VAL> {
public:
  explicit WDL_KeyedArrayImpl(void (*valdispose)(VAL) = NULL)
      : WDL_AssocArrayImpl<KEY, VAL>(WDL_assocarray_cmp<KEY>, NULL, NULL,
                                     valdispose) {}
};

template <class KEY, class VAL>
class WDL_MemKeyedArray : public WDL_AssocArray<KEY, VAL> {
public:
  explicit WDL_MemKeyedArray(void (*valdispose)(VAL) = NULL)
      : WDL_AssocArray<KEY, VAL>(WDL_assocarray_cmpmem<KEY>, NULL, NULL,
                                 valdispose) {}
};

template <class KEY, class VAL>
class WDL_MemKeyedArrayImpl : public WDL_AssocArrayImpl<KEY, VAL> {
public:
  explicit WDL_MemKeyedArrayImpl(void (*valdispose)(VAL) = NULL)
      : WDL_AssocArrayImpl<KEY, VAL>(WDL_assocarray_cmpmem<KEY>, NULL, NULL,
                                     valdispose) {}
};
...
public:
  explicit WDL_IntKeyedArray(void (*valdispose)(VAL) = NULL)
      : WDL_KeyedArray<int, VAL>(valdispose) {}
};

template <class VAL>
class WDL_IntKeyedArray2 : public WDL_KeyedArrayImpl<int, VAL> {
public:
  explicit WDL_IntKeyedArray2(void (*valdispose)(VAL) = NULL)
      : WDL_KeyedArrayImpl<int, VAL>(valdispose) {}
};

template <class VAL>
class WDL_StringKeyedArray : public WDL_AssocArray<const char *, VAL> {
public:
  explicit WDL_StringKeyedArray(bool caseSensitive = true,
                                void (*valdispose)(VAL) = NULL,
                                bool copyKeys = true)
      : WDL_AssocArray<const char *, VAL>(
            caseSensitive ? WDL_assocarray_cmpstr<const char>
                          : WDL_assocarray_cmpistr<const char>,
            copyKeys ? dupstr : NULL, copyKeys ? freestr : NULL, valdispose) {}
...
template <class VAL>
class WDL_StringKeyedArray2 : public WDL_AssocArrayImpl<const char *, VAL> {
public:
  explicit WDL_StringKeyedArray2(bool caseSensitive = true,
                                 void (*valdispose)(VAL) = NULL,
                                 bool copyKeys = true)
...
template <class VAL>
class WDL_LogicalSortStringKeyedArray : public WDL_StringKeyedArray<VAL> {
public:
  explicit WDL_LogicalSortStringKeyedArray(bool caseSensitive = true,
                                           void (*valdispose)(VAL) = NULL,
                                           bool copyKeys = true)
      : WDL_StringKeyedArray<VAL>(caseSensitive, valdispose, copyKeys) {
    WDL_StringKeyedArray<VAL>::m_keycmp =
        caseSensitive ? cmpstr : cmpistr; // override
  }

  ~WDL_LogicalSortStringKeyedArray() {}

  static int cmpstr(const char *const *a, const char *const *b) {
    int r = WDL_strcmp_logical_ex(*a, *b, 1,
                                  WDL_STRCMP_LOGICAL_EX_FLAG_UTF8CONVERT);
    return r ? r : strcmp(*a, *b);
  }
  static int cmpistr(const char *const *a, const char *const *b) {
    int r = WDL_strcmp_logical_ex(*a, *b, 0,
                                  WDL_STRCMP_LOGICAL_EX_FLAG_UTF8CONVERT);
...
template <class VAL>
class WDL_PtrKeyedArray : public WDL_KeyedArray<INT_PTR, VAL> {
public:
  explicit WDL_PtrKeyedArray(void (*valdispose)(VAL) = NULL)
      : WDL_KeyedArray<INT_PTR, VAL>(valdispose) {}
};

template <class KEY, class VAL>
class WDL_PointerKeyedArray : public WDL_KeyedArray<KEY, VAL> {
public:
  explicit WDL_PointerKeyedArray(void (*valdispose)(VAL) = NULL)
      : WDL_KeyedArray<KEY, VAL>(valdispose) {}
};

struct WDL_Set_DummyRec {};
template <class KEY>
class WDL_Set : public WDL_AssocArrayImpl<KEY, WDL_Set_DummyRec> {
public:
  explicit WDL_Set(int (*keycmp)(const KEY *k1, const KEY *k2),
                   KEY (*keydup)(KEY) = NULL, void (*keydispose)(KEY) = NULL)
  ...
  bool Enumerate(int i, KEY *key = NULL) {
    return WDL_AssocArrayImpl<KEY, WDL_Set_DummyRec>::EnumeratePtr(i, key) !=
           NULL;
  }
};
