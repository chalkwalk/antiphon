...
/*
  ...
  This file provides simple interfaces for encoding and decoding of OGG Vorbis
  data. It is a wrapper around what their SDKs expose, which is not too easy to
  use.
...
public:
  ...
  virtual void Encode(float *in, int inlen, int advance = 1,
                      int spacing = 1) = 0; // length in sample (PAIRS)
...
public:
  ...
  void *DecodeGetSrcBuffer(int srclen) { return ogg_sync_buffer(&oy, srclen); }
  ...
  int GenerateLappingSamples() {
    if (vd.pcm_returned < 0 || !vd.vi || !vd.vi->codec_setup) {
      return 0;
    }
    float **pcm;
...
public:
  ...
  VorbisEncoder(int srate, int nch, int serno, float qv, int cbr = -1,
                int minbr = -1, int maxbr = -1, const char *encname = NULL,
...
#endif
  {
    ...
    if (cbr > 0) {
      const int brlimit = (srate < 11025   ? 32
                          ...
                          nch;
      for (;;) {
        m_err = vorbis_encode_init(&vi, nch, srate, maxbr * 1000, cbr * 1000,
                                   minbr * 1000);
        if (!m_err)
          break;
    ...
    if (bitrate < 40)
      ...
      qv = -0.10f + (bitrate - 40) * (0.10f / 24.0f);
    else if (bitrate < 75)
      qv = (bitrate - 64) * (0.1f / 9.0f);
    else if (bitrate < 95)
      qv = 0.1f + (bitrate - 75) * (0.2f / 20.0f);
    else if (bitrate < 110)
      qv = 0.3f + (bitrate - 95) * (0.2f / 15.0f);
    else if (bitrate < 140)
      qv = 0.5f + (bitrate - 110) * (0.25f / 30.0f);
    else
      qv = 0.75f + (bitrate - 140) * (0.25f / 100.0f);
    ...
    vorbis_comment_init(&vc);
    if (encname)
      vorbis_comment_add_tag(&vc, "ENCODER", (char *)encname);
  ...
  void Encode(float *in, int inlen, int advance = 1,
              int spacing = 1) // length in sample (PAIRS)
  {
    if (m_err)
      return;
    ...
    while (vorbis_analysis_blockout(&vd, &vb) == 1) {
      ...
      while (vorbis_bitrate_flushpacket(&vd, &op)) {
        ...
        while (!eos) {
          ogg_page og;
          int result = m_flushmode ? ogg_stream_flush(&os, &og)
  ...
  int Available() { return outqueue.Available(); }
  void *Get() { return outqueue.Get(); }
  void Advance(int amt) { outqueue.Advance(amt); }

  void Compact() { outqueue.Compact(); }
