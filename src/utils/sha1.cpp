...
void WDL_SHA1::reset() {
  ...
  H[4] = 0xc3d2e1f0;
  int x;
  for (x = 0; x < (int)(sizeof(W) / sizeof(W[0])); x++)
    W[x] = 0;
}

#define SHA_ROTL(X, n)                                                         \
  ((((X) & 0xffffffff) << (n)) | (((X) & 0xffffffff) >> (32 - (n))))
#define SHUFFLE()                                                              \
  E = D;                                                                       \
  D = C;                                                                       \
  C = SHA_ROTL(B, 30);                                                         \
...
void WDL_SHA1::add(const void *data, int datalen) {
  ...
  for (i = 0; i < datalen; i++) {
    ...
    if (!(++lenW & 63)) {
      ...
      for (t = 0; t < 20; t++) {
        unsigned int TEMP =
            SHA_ROTL(A, 5) + E + W[t] + 0x5a827999 + (((C ^ D) & B) ^ D);
        SHUFFLE();
      }
      for (; t < 40; t++) {
        unsigned int TEMP =
            SHA_ROTL(A, 5) + E + W[t] + 0x6ed9eba1 + (B ^ C ^ D);
        SHUFFLE();
      }
      for (; t < 60; t++) {
        unsigned int TEMP =
            SHA_ROTL(A, 5) + E + W[t] + 0x8f1bbcdc + ((B & C) | (D & (B | C)));
        SHUFFLE();
      }
      for (; t < 80; t++) {
        unsigned int TEMP =
            SHA_ROTL(A, 5) + E + W[t] + 0xca62c1d6 + (B ^ C ^ D);
        SHUFFLE();
      }
