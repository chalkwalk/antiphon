...
public:
  ...
  void SwapContentsWith(
      WDL_HeapBuf *hb); // swaps members, avoids memcpy() of underlying data
#endif
  void *Get() const {
    return m_size ? m_buf : NULL;
  }                                       // returns NULL if size is 0
  void *GetFast() const { return m_buf; } // returns last buffer if size is 0
  int GetSize() const { return m_size; }
  void *GetAligned(int align) const {
    return (void *)(((UINT_PTR)Get() + (align - 1)) & ~(UINT_PTR)(align - 1));
  }
  ...
  void *ResizeOK(int newsize, bool resizedown = true) {
    void *p = Resize(newsize, resizedown);
...
#endif
  {
    ...
    if (newsize != m_size || (resizedown && newsize < m_alloc / 2)) {
      ...
      if (resizedown && newsize < m_size) {
        // shrinking buffer: only shrink if allocation decreases to min(alloc/2,
        // alloc-granul*4) or 0
        resizedown_under = m_alloc - (m_granul << 2);
        if (resizedown_under > m_alloc / 2)
          resizedown_under = m_alloc / 2;
        if (resizedown_under < 1)
          resizedown_under = 1;
      }
      ...
      if (newsize > m_alloc || newsize < resizedown_under) {
        ...
        if (newsize < 1)
          newalloc = 0;
        else if (m_granul < 4096)
          newalloc = newsize + granul;
        else {
          granul &= ~4095;
          if (granul < 4096)
            granul = 4096;
          else if (granul > 4 * 1024 * 1024)
            granul = 4 * 1024 * 1024;
        ...
        if (newalloc != m_alloc) {
          ...
          if (m_buf)
            ...
            WDL_HEAPBUF__TRACE_ACTION(heapbuf_malloc);
#endif
...
#endif
  {
    if (exactCopyOfConfig) // copy all settings
    {
      ...
      m_size = m_alloc = 0;
      m_buf =
          hb->m_buf && hb->m_alloc > 0 ? malloc(m_alloc = hb->m_alloc) : NULL;
#ifdef WDL_HEAPBUF_ONMALLOCFAIL
      if (!m_buf && m_alloc) {
...
#endif
      if (m_buf)
        memcpy(m_buf, hb->m_buf, m_size = hb->m_size);
      else
        m_alloc = 0;
    } else // copy just the data + size
    {
      ...
      if (GetSize() != newsz)
        ...
        memcpy(Get(), hb->Get(), newsz);
    }
  }
#ifdef WDL_HEAPBUF_IMPL_ONLY
  void WDL_HeapBuf::SwapContentsWith(
      WDL_HeapBuf *hb) // swaps members, avoids memcpy() of underlying data
#else
...
#endif
  {
    ...
    if (WDL_NOT_NORMALLY(!hb || hb == this))
      return;
...
public:
  ...
  int GetAlloc() const {
    return m_hb.GetAlloc() / (unsigned int)sizeof(PTRTYPE);
  }

  PTRTYPE *Resize(int newsize, bool resizedown = true) {
    return (PTRTYPE *)m_hb.Resize(newsize * sizeof(PTRTYPE), resizedown);
  }
  PTRTYPE *ResizeOK(int newsize, bool resizedown = true) {
    return (PTRTYPE *)m_hb.ResizeOK(newsize * sizeof(PTRTYPE), resizedown);
  }
  ...
  PTRTYPE *GetAligned(int align) const {
    return (PTRTYPE *)m_hb.GetAligned(align);
  }
  ...
  PTRTYPE *Add(const PTRTYPE *buf, int bufsz) {
    if (bufsz > 0) {
      ...
      if (p) {
        p += sz;
        if (buf)
          memcpy(p, buf, bufsz * sizeof(PTRTYPE));
        else
          memset((char *)p, 0, bufsz * sizeof(PTRTYPE));
        return p;
      }
    }
    return NULL;
  ...
  void DeleteRange(int index, int count) {
    ...
    if (list && count > 0 && index >= 0 && index < size) {
      ...
      size -= count;
      if (index < size)
        memmove(list + index, list + index + count,
                (unsigned int)sizeof(PTRTYPE) * (size - index));
      Resize(size, false);
    }
  }
  ...
  int DeleteBatch(bool (*proc)(PTRTYPE *p, void *ctx),
                  void *ctx = NULL) // proc returns true to delete item. returns
                                    // number deleted
  {
    const int sz = GetSize();
    int cnt = 0;
  ...
  void SwapContentsWith(WDL_TypedBuf<PTRTYPE> *b) {
    m_hb.SwapContentsWith(&b->m_hb);
  }
